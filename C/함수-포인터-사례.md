...
2. 구조체를 이용한 객체지향(OOP) 흉내내기 (Polymorphism) => 다형성
- 구조체는 함수를 넣지 못한다.
- 대신 구조체 안에 함수 포인터를 넣어 메서드(Method)처럼 사용할 수 있다.
    - 리눅스 커널의 VFS(가상 파일 시스템)이 대표적인 예
    - `file_operations` 구조체에는 `read`, `write`, `open` 등의 함수 포인터를 갖고있음
    - 디바이스 드라이버마다 실제 동작 함수를 다르게 연
