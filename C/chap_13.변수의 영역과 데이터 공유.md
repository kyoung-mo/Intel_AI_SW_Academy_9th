...
## 정적(static) 변수
- `main()` 함수 외에 다른 함수 실행 시, 정적 지역 변수를 사용하면 함수가 종료되어도 정적 지역 변수가 남아있을 수 있기 때문에, 함수의 반환값으로 정적 지역변수의 주소 값을 반환해줄 수 있다.
- 함수 내의 일반 지역 변수는 스택에 생성되고 함수 종료 시 반환
- 함수 내에서 정적 변수 사용 시 전역 영역인 Data Segment에 저장된다.
- Data Segment 영역은 프로그램 종료 전까지 메모리에서 사라지지 않는다.


## 주 메모리(RAM) 내부 구조 정리 (실행 중)
```css
[코드 영역]
[전역 변수 영역]
[힙 영역]
