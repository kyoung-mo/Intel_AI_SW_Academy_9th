...
<img width="906" height="83" alt="image" src="https://github.com/user-attachments/assets/09557f84-6f57-45c1-8572-ecb51d2871ff" />

### 요약: 핵심 옵션 설명

| **옵션** | **설명** | **예시** |
| --- | --- | --- |
| **`-I<경로>`** | **Include Path**. 컴파일러가 헤더 파일(.h)을 찾을 위치 지정 | `-I./include` |
| **`-L<경로>`** | **Library Path**. 링커가 라이브러리 파일(.a, .so)을 찾을 위치 지정 | `-L./lib` |
| **`-l<이름>`** | **Library Name**. 링크할 라이브러리 이름 지정 (`lib` 접두사와 확장자 제외) | `-lmymath` (실제 파일명:   -libc, -lmath
`libmymath.a` 또는 `libmymath.so`) |
| **`-fPIC`** | **Position Independent Code**. 메모리 위치에 상관없이 실행 가능한 코드 생성 (공유 라이브러리 필수) |  |
| **`-shared`** | 공유 라이브러리(.so) 생성 옵션 |  |
|  |  |  |

### add.c, substract 파일에서 mymath.h를 인클루드 하는 이유는?

`mymath.c` (구현 파일)에서 `mymath.h` (헤더 파일)를 인클루드하는 것은 C언어 프로그래밍에서 매우 중요한 **표준 규약(Best Practice)**입니다. 그 이유는 크게 두 가지입니다.

### 1. 선언과 정의의 일치 여부 검증 (Safety & Validation)

가장 큰 이유는 **"내가 약속한 대로 만들었는지 컴파일러가 검사하게 하기 위해서"**입니다.

- *헤더 파일(`mymath.h`)**에는 함수의 ****원형(Prototype**)**이 들어있습니다. (일종의 약속)
- *소스 파일(`mymath.c`)**에는 함수의 ****구현(Definition)****이 들어있습니다. (실제 행동)

만약 `mymath.c`가 헤더를 포함하지 않으면, 실수로 함수의 매개변수나 반환 타입을 다르게 작성해도 컴파일러가 이를 알아채지 못하고 `.o` 파일을 만들어버릴 수 있습니다. 이는 나중에 프로그램을 실행할 때 심각한 오류를 발생시킵니다.
...
- **mymath.h (약속):**C
    ...
    ```c
    int add(int a, int b); // 인자를 2개 받겠다고 선언
    ```
    
- **mymath.c (실수):**C
    
    ```c
    // #include "mymath.h"를 깜빡함!
...
위 경우, 헤더를 인클루드하지 않으면 `mymath.c`를 컴파일할 때(`gcc -c ...`) 에러가 나지 않을 수 있습니다. 하지만 나중에 이 라이브러리를 사용하는 쪽에서는 인자를 2개만 넣으려 할 것이고, 결국 프로그램은 엉뚱한 동작을 하거나 충돌합니다.

**헤더를 인클루드 했다면:**
컴파일러가 `mymath.h`의 선언과 `mymath.c`의 구현을 비교하여 **"어? 아까는 인자 2개라고 해놓고 왜 3개짜리를 만드나요?"** 라며 즉시 **컴파일 에러(Conflict type error)**를 띄워줍니다.

### 2. 공통 자료형 및 매크로 사용 (Dependency)

헤더 파일에 단순 함수 선언뿐만 아니라 **구조체(struct), 열거형(enum), 매크로(#define)** 등이 정의되어 있는 경우입니다.

구현 파일(`mymath.c`)에서 이 타입들을 사용하여 코드를 작성해야 하므로, 당연히 해당 정의가 들어있는 헤더를 인클루드해야 합니다.
...
- **mymath.c**
    ...
    ```c
    #include "mymath.h" // 이게 없으면 PI와 Point가 뭔지 몰라서 에러 발생
...
프로젝트 루트 폴더(`my_project`)에 `Makefile`이라는 이름의 파일을 만들고 아래 내용을 붙여넣으시면 됩니다.
...
# 1. 컴파일러 및 옵션 설정
CC = gcc
# CFLAGS: -Wall(경고 표시), -I(헤더 경로), -fPIC(공유 라이브러리용 위치 독립 코드)
...
# 4. 기본 타겟 (make만 쳤을 때 실행됨)
all: directories $(EXE_STATIC) $(EXE_SHARED)

# 5. 디렉토리 생성 (bin 폴더가 없을 경우 대비)
directories:
	@mkdir -p $(BIN_DIR)
	@mkdir -p $(LIB_DIR)
...
# (1) 라이브러리 오브젝트 파일 생성
# $< : 첫 번째 의존성 파일 (src/mymath.c)
# $@ : 타겟 파일 (src/mymath.o)
$(SRC_DIR)/mymath.o: $(SRC_DIR)/mymath.c
	$(CC) $(CFLAGS) -c $< -o $@

# (2) 정적 라이브러리 (.a) 생성
$(LIB_STATIC): $(SRC_DIR)/mymath.o
	ar rcs $@ $<

# (3) 공유 라이브러리 (.so) 생성
$(LIB_SHARED): $(SRC_DIR)/mymath.o
	$(CC) -shared -o $@ $<

# (4) 정적 라이브러리를 사용한 실행 파일 빌드
# main.c와 libmymath.a가 있어야 실행됨
$(EXE_STATIC): $(APP_DIR)/main.c $(LIB_STATIC)
	$(CC) $(CFLAGS) $< -o $@ -L$(LIB_DIR) -lmymath

# (5) 공유 라이브러리를 사용한 실행 파일 빌드
# main.c와 libmymath.so가 있어야 실행됨
...
터미널에서 `my_project` 폴더로 이동한 뒤 다음 명령어들을 사용하세요.
...
1. `src/mymath.c`를 컴파일해 `src/mymath.o`를 만듭니다.
2. `lib` 폴더에 `.a`와 `.so` 파일을 만듭니다.
3. `bin` 폴더에 `main_static`과 `main_shared` 실행 파일을 만듭니다.
...
- **공유 링크 실행 파일:**
(이전과 마찬가지로 라이브러리 경로를 알려줘야 합니다)Bash
...
Makefile 안에 사용된 기호들의 의미입니다. 이걸 알면 나중에 수정하기 편합니다.

- `$@` (Target): 현재 규칙의 **결과물 파일 이름**입니다.
    - 예: `$(CC) -o $@` → `gcc -o bin/main_static`
- `$<` (First Prerequisite): 현재 규칙의 **첫 번째 재료 파일**입니다.
    - 예: `$(CC) -c $<` → `gcc -c src/mymath.c`
- `^` (All Prerequisites): 모든 재료 파일입니다.

이 Makefile은 소스 코드가 수정되면, 변경된 부분만 알아서 다시 컴파일하도록 의존성(`Dependency`)이 설정되어 있습니다. 예를 들어 `src/mymath.c`를 고치고 `make`를 치면, 라이브러리와 실행 파일까지 싹 업데이트됩니다.

### 위 예제를 cmake로 하기

CMake를 사용하면 `gcc` 명령어를 직접 칠 필요 없이, 설정 파일(`CMakeLists.txt`) 하나로 복잡한 컴파일과 링크 과정을 관리할 수 있습니다.

---

### 1. 프로젝트 구조 확인

먼저 파일들이 다음과 같이 위치해 있다고 가정합니다.
...
프로젝트 최상위 폴더(my_project)에 CMakeLists.txt 파일을 만들고 아래 내용을 작성하세요.

이 파일이 기존의 gcc, ar, -I, -L 옵션들을 대신합니다.
...
add_library(mymath_static STATIC src/mymath.c)
...
add_library(mymath_shared SHARED src/mymath.c)
...
### 3. 빌드 실행 (Out-of-source Build)

CMake의 장점은 소스 코드 폴더를 더럽히지 않고 별도의 `build` 폴더에서 빌드할 수 있다는 점입니다.

터미널에서 `my_project` 폴더로 이동한 후 아래 명령어를 순서대로 입력하세요.
...
빌드가 완료되면 `build` 폴더 내부에 우리가 설정한 대로 결과물이 생성됩니다.
...
이건 바로 실행됩니다. 라이브러리가 안에 포함되어 있기 때문입니다.
...
공유 라이브러리(.so)는 실행 시점에 라이브러리를 찾아야 합니다.

CMakeLists.txt가 빌드된 위치(build/lib)를 가리키고 있지만, 배포 환경 등을 고려해 LD_LIBRARY_PATH를 지정해주는 것이 명확합니다.
...
### 명령어 매핑 요약 (Manual vs CMake)

| **수동 명령어 (GCC)** | **CMake 명령어** | **비고** |
| --- | --- | --- |
| `gcc -c ... -I./include` | `include_directories(...)` | 헤더 경로 추가 |
| `gcc -fPIC` | `add_library(... SHARED ...)` | SHARED 옵션 시 자동 적용 |
| `ar rcs libmymath.a ...` | `add_library(... STATIC ...)` | 정적 라이브러리 생성 |
| `gcc -shared ...` | `add_library(... SHARED ...)` | 공유 라이브러리 생성 |
| `gcc ... -L./lib -lmymath` | `target_link_libraries(...)` | 라이브러리 링크 |

### Tip

CMake를 사용하면 `make clean` 명령어 역시 자동으로 지원되므로, `build` 폴더 안에서 `make clean`을 입력하면 빌드된 파일들을 싹 지울 수 있습니다. 혹은 그냥 `build` 폴더 전체를 삭제(`rm -rf build`)해도 소스 코드에는 아무런 영향이 없습니다.
